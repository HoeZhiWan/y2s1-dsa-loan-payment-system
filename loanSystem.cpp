...
User* LoanSystem::login(const string& userId, const string& password) {
    for (auto& user : users) {
        if (user.getUserId() == userId && user.getPasswordHash() == password) {
            return &user;
        }
    }
    return nullptr;
...
void LoanSystem::createLoan() {
    ...
    cout << "\n--- Create New Loan ---\n";
    int next = static_cast<int>(loans.size()) + 1;
...
void LoanSystem::viewUpcomingPayments() {
    cout << "\n--- Upcoming Payments ---\n";
    if (loans.empty()) {
...
void LoanSystem::manageUsers() {
    ...
    if (choice == 1) {
        ...
        cout << "Role (ADMIN/BORROWER): "; getline(cin, roleStr);

        int nextU = static_cast<int>(users.size()) + 1;
        ...
        string delId; cout << "Enter User ID to delete: "; cin >> delId;
        auto it = remove_if(users.begin(), users.end(), [&](const User& u){ return u.getUserId() == delId; });
        if (it != users.end()) {
